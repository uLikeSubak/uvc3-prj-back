...
class AttendList extends Sequelize.Model {
  // 스태틱 메소드
  // 테이블에 대한 설정
    ...
    return super.init(
      {
        ...
        status: {
          ...
          allowNull: false,
          defaultValue: true, // 신청하는대로 바로 참석확정자로 인정
        },
      },
      {
        ...
        modelName: "AttendList" /* 모델 이름을 설정. */,
        tableName: "attendLists" /* 데이터베이스의 테이블 이름. */,
  ...
  static associate(db) {
    ...
    db.AttendList.belongsTo(db.Post);
    db.AttendList.belongsTo(db.User);
  }
}

module.exports = AttendList;
